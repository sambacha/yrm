...
In that case, you will need to use a fake local domain (`dev.yfinance.dev`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).
...
To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.yfinance.dev`.

After performing those steps you should be able to open: http://dev.yfinance.dev and it will be server by your stack in `localhost`.

Check all the corresponding available URLs in the section at the end.

### Change the development "domain"

If you need to use your local stack with a different domain than `localhost`, you need to make sure the domain you use points to the IP where your stack is set up. See the different ways to achieve that in the sections above (i.e. using Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or using `dev.yfinance.dev`).
...
```bash
STACK_NAME=stag-yfinance-dev sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.yfinance-dev.app-db-data == true
```
...
Frontend: https://yfinance.dev

Backend: https://yfinance.dev/api/

Automatic Interactive Docs (Swagger UI): https://yfinance.dev/docs

Automatic Alternative Docs (ReDoc): https://yfinance.dev/redoc

PGAdmin: https://pgadmin.yfinance.dev

Flower: https://flower.yfinance.dev

### Staging URLs

Staging URLs, from the branch `master`.

Frontend: https://stag.yfinance.dev

Backend: https://stag.yfinance.dev/api/

Automatic Interactive Docs (Swagger UI): https://stag.yfinance.dev/docs

Automatic Alternative Docs (ReDoc): https://stag.yfinance.dev/redoc

PGAdmin: https://pgadmin.stag.yfinance.dev

Flower: https://flower.stag.yfinance.dev
...
Frontend: http://dev.yfinance.dev

Backend: http://dev.yfinance.dev/api/

Automatic Interactive Docs (Swagger UI): https://dev.yfinance.dev/docs

Automatic Alternative Docs (ReDoc): https://dev.yfinance.dev/redoc

PGAdmin: http://dev.yfinance.dev:5050

Flower: http://dev.yfinance.dev:5555

Traefik UI: http://dev.yfinance.dev:8090
